/**
 * GET /api/player/warehouse-marketing-campaigns?warehouseBuildingId=...
 * POST /api/player/warehouse-marketing-campaigns - create campaign (packageId, ledger + wallet)
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const campaigns = await (prisma as any).warehouseMarketingCampaign.findMany({
      where: {
    ...
    return NextResponse.json({
      campaigns: campaigns.map((c: { startDayKey: Date; endDayKey: Date; totalPriceSnapshot: unknown; createdAt: Date; [k: string]: unknown }) => ({
        id: c.id,
    ...
    console.error('[warehouse-marketing-campaigns GET]', e);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { warehouseBuildingId, packageId, title, notes } = body as {
    ...
    const pkg = await getPackage(prisma, packageId, 'WAREHOUSE');
    ...
    const overlapping = await (prisma as any).warehouseMarketingCampaign.findFirst({
      where: {
        companyId: company.id,
        warehouseBuildingId,
    ...
    if (overlapping) {
      return NextResponse.json(
        { error: 'A campaign already exists for this warehouse in the selected period.' },
        { status: 409 }
      );
    }

    const skuCount = await getWarehouseSkuCount(prisma, company.id, warehouseBuildingId);
    const multiplierStr = await getPricingMultiplier(prisma, 'WAREHOUSE', skuCount);
    ...
    const result = await prisma.$transaction(
      async (tx) => {
        const campaign = await (tx as any).warehouseMarketingCampaign.create({
          data: {
            companyId: company.id,
            warehouseBuildingId,
            startDayKey,
        ...
        const idempotencyKey = `MKT:WAREHOUSE:${campaign.id}`;
        await postLedgerEntryAndUpdateWallet(tx, userId, {
          ...
          refId: campaign.id,
          idempotencyKey,
          note: `Warehouse campaign ${pkg.key} SKU ${skuCount}`,
        });
    ...
    console.error('[warehouse-marketing-campaigns POST]', e);
