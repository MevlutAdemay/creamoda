...
export default async function WarehousePage({ searchParams }: PageProps) {
  ...
  const stockRow = buildMetricRow('STOCK_COUNT', stockState, levelConfigRows);
  const salesRow = buildMetricRow('SALES_COUNT', salesState, levelConfigRows);
          ...
          {warehouses.length === 0 ? (
            <Card className="border bg-card shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Add a warehouse to start receiving stock and tracking capacity.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button variant="outline" size="sm" disabled>
                  Add warehouse (coming soon)
                </Button>
              </CardContent>
            </Card>
          ) : (
                      ...
                      <CardContent>
                        {recentMessages.length === 0 ? (
                          <p className="text-sm text-muted-foreground">
                            No recent messages for this warehouse.
                          </p>
                        ) : (
