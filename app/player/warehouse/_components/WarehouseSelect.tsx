...
type WarehouseSelectProps = {
  ...
  /** Params to preserve when only buildingId changes (e.g. { tab: 'inventory' }) */
...
export function WarehouseSelect({
  ...
  const clearToFirst = (e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute top-full right-0 mt-2 min-w-[200px] w-full max-w-[220px] bg-popover border border-border rounded-md shadow-lg z-20 py-1 max-h-[280px] overflow-y-auto">
          {warehouses.map((w) => (
            <button
              key={w.id}
              type="button"
